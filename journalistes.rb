...
def number(journalists)
    x = 0
    y = 0
    while (y <= 9)    
        journalists.each do |i|
            boolean = i.include?(y.to_s)
            if boolean == true                
                x = x + 1
            end
        end
    ...
    puts "Il y a #{x} handle contenant un numéro"
end

number(journalists)

def aude(journalists)
	x = 0
	journalists.each do |i|
		boolean = i.include?("aude")
		if boolean == true
			x = x + 1
		end
	end
	puts "Il y a #{x} handle contenant le prénom Aude" 
end

aude(journalists)


	def start_majuscule(journalists)
		x = 0 
		journalists.each do |i|
			y = i[1].scan(/[A-Z]/).length
			if y > 0
				x = x + 1
			end
		end
...
start_majuscule(journalists)

def trie(journalists)
	journalists.sort!
	puts journalists
end
...
def underscore(journalists)
	x = 0
	journalists.each do |journalist|
		boolean = journalist.include?("_")
		if boolean == true
			x = x + 1
		end
	end
	puts "Il y a #{x} underscore dans tous les pseudos confondus" 
end
...
def majuscule(journalists)
	x = 0 
	journalists.each do |journalist|
		boolean = journalist.include?(A-Z)
		if boolean == true
			x = x +1
		end
	end
